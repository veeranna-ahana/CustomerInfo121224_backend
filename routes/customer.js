          ...
          setupQuery(
            ...
            async (runningno) => {
              ...
              let qno = (parseInt(runningno[0]["Running_No"]) + 1)
                ...
                .padStart(4, "0");
              console.log(qno);
              createFolder("Customer", qno, newQno, month, (err, fres) => {
                if (err) logger.error(err);
              });
          ...
          misQuery(
            ...
            async (response2) => {
              ...
              if (foldername != custcode) {
                createFolder("Customer", dwgfolder, foldername, month, (err, fres) => {
                  if (err) logger.error(err);
                  copyallfiles("Customer", custcode, dwgfolder); // foldername);
                });
              }
              //else {
              //   const foldername = req.body.custfoldername;
  ...
  try {
    ...
    misQueryMod(`Select cad.Id as custassyid from magodmis.cust_assy_data cad where cad.AssyCust_PartId = '${selectedbomdata.assyPartId}'`, (err, asmdata) => {
      ...
      misQueryMod(`SELECT cb.Id as custbomId FROM magodmis.cust_bomlist as cb where PartId = '${selectedbomdata.partid}' and Cust_code='${ccode}'`, (err, asmprtdata) => {
        ...
        console.log(asmprtdata[0].custbomId);
        misQueryMod(
  ...
  await misQueryMod('SELECT * FROM magod_setup.magodlaser_units', (err, unitData) => {
    ...
    misQueryMod(`SELECT DateDiff(Curdate(),d.Inv_Date) as DueDays,d.DC_Inv_No,d.IsDC,d.ScheduleId,DATE_FORMAT(d.Dc_inv_Date, "%d/%l/%Y") AS 'Dc_inv_Date' ,d.DC_InvType,
      ...
      if (newData.length > 0) {
        ...
        doc.fontSize(12).text(unitData[0].RegisteredName, textX, textY, { align: 'left', textDecoration: 'bold' });
        doc.fontSize(10).text(unitData[0].Unit_Address, textX, textY + 15, { align: 'left' }); // 15 units down from the RegisteredName 
        doc.moveDown(1); // Add some space before the next section

        doc.fontSize(10).text(`List of Invoices Due For Payment as on: ${ason}`, { align: 'center' });
        ...
        function printTableHeaders(doc, headerX, tableTop) {
          ...
          doc.moveTo(30, tableTop).lineTo(560, tableTop).stroke();
  ...
  await misQueryMod('SELECT * FROM magod_setup.magodlaser_units', async (err, unitData) => {
    ...
    await misQueryMod(
      ...
      (err, newData) => {
        ...
        doc.fontSize(12).text(`Outstanding Invoices Report For ${req.body.custname}`, { align: 'center' });
